...
kustomizedyff() {
  ...
  if git_root="$(git rev-parse --show-toplevel 2>/dev/null)"; then
    ...
    if [[ -n "$old_ref" ]]; then
      ...
      if git -C "$git_root" archive "$old_ref" | tar -x -C "$tmpdir"; then
        old_dir="$tmpdir/$rel_dir"
        if ! (cd "$old_dir" && kustomize build .) > "$old_out"; then
          rm -rf "$tmpdir"
          return 1
        fi
      else
        echo "Could not read $old_ref; leaving $old_out empty." >&2
      fi
  ...
  dyff between "$old_out" "$new_out" > "$dyff_out" || true
  echo "Wrote $old_out, $new_out, and $dyff_out"
}
